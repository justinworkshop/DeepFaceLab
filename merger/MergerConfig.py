...
class MergerConfig(object):
    ...
    def to_string(self, filename):
        r = ""
        r += f"锐化模式 sharpen_mode: {self.sharpen_dict[self.sharpen_mode]}\n"
        r += f"锐化力度blursharpen_amount: {self.blursharpen_amount}\n"
        return r
...
class MergerConfigMasked(MergerConfig):
    ...
    def ask_settings(self):
        ...
        if 'raw' not in self.mode:
            self.erode_mask_modifier = np.clip ( io.input_int ("遮罩大小 Choose erode mask modifier", 0, add_info="-400..400"), -400, 400)
            self.blur_mask_modifier =  np.clip ( io.input_int ("遮罩羽化 Choose blur mask modifier", 0, add_info="0..400"), 0, 400)
            self.motion_blur_power = np.clip ( io.input_int ("动态模糊 Choose motion blur power", 0, add_info="0..100"), 0, 100)

        self.output_face_scale = np.clip (io.input_int ("头像缩放 Choose output face scale modifier", 0, add_info="-50..50" ), -50, 50)

        if 'raw' not in self.mode:
            self.color_transfer_mode = io.input_str ( "颜色转换 Color transfer to predicted face", None, valid_list=list(ctm_str_dict.keys())[1:] )
            self.color_transfer_mode = ctm_str_dict[self.color_transfer_mode]

        super().ask_settings()

        self.super_resolution_power = np.clip ( io.input_int ("超分力度 Choose super resolution power", 0, add_info="0..100", help_message="Enhance details by applying superresolution network."), 0, 100)

        if 'raw' not in self.mode:
            self.image_denoise_power = np.clip ( io.input_int ("图片降噪力度 Choose image degrade by denoise power", 0, add_info="0..500"), 0, 500)
            self.bicubic_degrade_power = np.clip ( io.input_int ("双三次插值降低力度 Choose image degrade by bicubic rescale power", 0, add_info="0..100"), 0, 100)
            self.color_degrade_power = np.clip (  io.input_int ("颜色降低力度 Degrade color power of final image", 0, add_info="0..100"), 0, 100)
    ...
    def to_string(self, filename):
        r = (
            f"""合成图片 MergerConfig {filename}:\n"""
            f"""合成模式 Mode: {self.mode}\n"""
            )

        if self.mode == 'hist-match':
            r += f"""直方图蒙版 masked_hist_match: {self.masked_hist_match}\n"""

        if self.mode == 'hist-match' or self.mode == 'seamless-hist-match':
            r += f"""亮度 hist_match_threshold: {self.hist_match_threshold}\n"""

        r += f"""遮罩模式 mask_mode: { mask_mode_dict[self.mask_mode] }\n"""

        if 'raw' not in self.mode:
            r += (f"""遮罩大小 erode_mask_modifier: {self.erode_mask_modifier}\n"""
                  f"""遮罩羽化 blur_mask_modifier: {self.blur_mask_modifier}\n"""
                  f"""动态模糊 motion_blur_power: {self.motion_blur_power}\n""")

        r += f"""头像缩放 output_face_scale: {self.output_face_scale}\n"""

        if 'raw' not in self.mode:
            r += f"""颜色模式 color_transfer_mode: {ctm_dict[self.color_transfer_mode]}\n"""
            r += super().to_string(filename)

        r += f"""超分力度 super_resolution_power: {self.super_resolution_power}\n"""

        if 'raw' not in self.mode:
            r += (f"""图片降噪力度 image_denoise_power: {self.image_denoise_power}\n"""
                  f"""双三次插值降低力度 bicubic_degrade_power: {self.bicubic_degrade_power}\n"""
                  f"""颜色降低力度color_degrade_power: {self.color_degrade_power}\n""")
