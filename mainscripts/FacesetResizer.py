        ...
        def process_data(self, filepath):
            try:
                ...
                if dflimg is None or not dflimg.has_data():
                    self.log_err (f"{filepath.name} 不符合DFL规范的图片")
                else:
...
def process_folder ( dirpath):
    ...
    image_size = io.input_int(f"新图片大小 New image size", 512, valid_range=[128,2048])
    
    face_type = io.input_str ("选择脸类型 Change face type", 'same', ['h','mf','f','wf','head','same']).lower()
    if face_type == 'same':
        face_type = None
    else:
    ...
    io.log_info (f"正在处理 {dirpath_parts} 文件夹")
    io.log_info ( f"文件将保存到 {output_dirpath_parts} 处理中....")
    ...
    if is_merge:
        io.log_info (f"把已处理完的文件拷贝到 {dirpath_parts}")
