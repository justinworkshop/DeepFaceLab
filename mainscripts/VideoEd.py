...
def extract_video(input_file, output_dir, output_ext=None, fps=None):
    ...
    if fps is None:
        fps = io.input_int ("一秒钟几张图 Enter FPS", 0, help_message="How many frames of every second of the video will be extracted. 0 - full fps")

    if output_ext is None:
        output_ext = io.input_str ("输出图片格式 Output image format", "png", ["png","jpg"], help_message="png is lossless, but extraction is x10 slower for HDD, requires x10 more disk space than jpg.")
...
def cut_video ( input_file, from_time=None, to_time=None, audio_track_id=None, bitrate=None):
    ...
    if bitrate is None:
        bitrate = max (1, io.input_int ("输出码率 Bitrate of output file in MB/s", 25) )
...
def denoise_image_sequence( input_dir, ext=None, factor=None ):
    ...
    for i,filepath in io.progress_bar_generator( enumerate(image_paths), "重命名", leave=False):
        src = filepath
...
def video_from_sequence( input_dir, output_file, reference_file=None, ext=None, fps=None, bitrate=None, include_audio=False, lossless=None ):
    ...
    if not lossless and bitrate is None:
        bitrate = max (1, io.input_int ("输出码率 Bitrate of output file in MB/s", 16) )
