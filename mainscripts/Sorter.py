...
class BlurEstimatorSubprocessor(Subprocessor):
    ...
    def process_info_generator(self):
        ...
        io.log_info(f'运行在 {cpu_count} CPUs')
...
class HistSsimSubprocessor(Subprocessor):
    ...
    def process_info_generator(self):
        ...
        io.log_info(f'运行在 {cpu_count} 线程')
        for i in range(cpu_count):
            yield 'CPU%d' % (i), {'i':i}, {}
...
sort_func_methods = {
    ...
    'motion-blur': ("动态模糊 motion_blur", sort_by_motion_blur),
    'face-yaw':    ("侧脸角度 face yaw direction", sort_by_face_yaw),
    'face-pitch':  ("俯仰角度 face pitch direction", sort_by_face_pitch),
    'face-source-rect-size' : ("脸部区域大小 face rect size in source image", sort_by_face_source_rect_size),
    'hist':        ("相似度 histogram similarity", sort_by_hist),
    'hist-dissim': ("差异度 histogram dissimilarity", sort_by_hist_dissim),
    'brightness':  ("明亮程度 brightness", sort_by_brightness),
