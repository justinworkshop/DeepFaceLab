...
def apply_xseg(input_path, model_path):
    if not input_path.exists():
        raise ValueError(f'{input_path} 不存在. 请先确保这个里有东西')

    if not model_path.exists():
        raise ValueError(f'{model_path} 不存在. 请先确保这个里有东西')
    ...
    if face_type is None:
        face_type = io.input_str ("遮罩模型人脸类型选择", 'same', ['h','mf','f','wf','head','same'], help_message="Specify face type of trained XSeg model. For example if XSeg model trained as WF, but faceset is HEAD, specify WF to apply xseg only on WF part of HEAD. Default is 'same'").lower()
        if face_type == 'same':
            face_type = None
    ...
    io.log_info(f'把已经训练好的模型应用到 {input_path.name}/ 文件夹。')
...
def fetch_xseg(input_path):
    ...
    io.log_info(f'文件已经拷贝: {len(files_copied)}')
...
def remove_xseg(input_path):
    ...
    for filepath in io.progress_bar_generator(images_paths, "处理中"):
        ...
        if dflimg.has_xseg_mask():
            ...
            dflimg.save()
            files_processed += 1
    io.log_info(f'文件已处理: {files_processed}')
