...
def main (model_class_name=None,
    ...
    try:
        ...
        is_interactive = io.input_bool ("是否启用交互式合成?", True) if not io.is_colab() else False

        if not is_interactive:
            cfg.ask_settings()
            
        subprocess_count = io.input_int("线程数量?", max(8, multiprocessing.cpu_count()), 
        ...
        if cfg.type == MergerConfig.TYPE_MASKED:
            ...
            if packed_samples is not None:
                ...
                def generator():
                    for sample in io.progress_bar_generator( packed_samples, "收集对齐头像"):
                        filepath = Path(sample.filename)
                        yield filepath, DFLIMG.load(filepath, loader_func=lambda x: sample.read_raw_file()  )
            else:
                def generator():
                    for filepath in io.progress_bar_generator( pathex.get_image_paths(aligned_path), "收集对齐头像"):
            ...
            if multiple_faces_detected:
                ...
                for i in io.progress_bar_generator( range(len(frames)) , "计算运动矢量"):
