...
class XSegModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if not self.is_first_run() and ask_override:
            if io.input_bool(f"重启训练?", False, help_message="Reset model weights and start training from scratch."):
                self.set_iter(0)
        ...
        if self.is_first_run():
            self.options['face_type'] = io.input_str ("人脸类型 Face type", default_face_type, ['h','mf','f','wf','head'], help_message="Half / mid face / full face / whole face / head. Choose the same as your deepfake model.").lower()

        if self.is_first_run() or ask_override:
            self.ask_batch_size(4, range=[2,16])
            self.options['pretrain'] = io.input_bool ("是否启用预训练？", default_pretrain)
    ...
    def export_dfm (self):
        ...
        io.log_info(f'导出 .onnx 文件到 {output_path}')
        tf = nn.tf
