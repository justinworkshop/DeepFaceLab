...
class ModelBase(object):
    ...
    def ask_override(self):
        return self.is_training and self.iter != 0 and io.input_in_time ("两秒内按回车可以修改模型参数...", 5 if io.is_colab() else 2 )
    ...
    def ask_write_preview_history(self, default_value=False):
        ...
        if self.options['write_preview_history']:
            if io.is_support_windows():
                ...
                self.choose_preview_history = io.input_bool("随机选择预览图 Randomly choose new image for preview history", False, help_message="Preview image history will stay stuck with old faces if you reuse the same model on different celebs. Choose no unless you are changing src/dst to a new person")
    ...
    def ask_random_src_flip(self):
        ...
        self.options['random_src_flip'] = io.input_bool("随机翻转源人脸 Flip SRC faces randomly", default_random_src_flip, help_message="Random horizontal flip SRC faceset. Covers more angles, but the face may look less naturally.")

    def ask_random_dst_flip(self):
        default_random_dst_flip = self.load_or_def_option('random_dst_flip', True)
        self.options['random_dst_flip'] = io.input_bool("随机翻转目标人脸 Flip DST faces randomly", default_random_dst_flip, help_message="Random horizontal flip DST faceset. Makes generalization of src->dst better, if src random flip is not enabled.")
    ...
    def ask_batch_size(self, suggest_batch_size=None, range=None):
        ...
        batch_size = max(0, io.input_int("批量大小 Batch_size", default_batch_size, valid_range=range, help_message="Larger batch size is better for NN's generalization, but it can cause Out of Memory error. Tune this value for your videocard manually."))
    ...
    def should_save_preview_history(self):
        return (not io.is_colab() and self.iter % 1000 == 0) or (io.is_colab() and self.iter % 1000 == 0)
    ...
    def get_loss_history_preview(loss_history, iter, w, c):
        ...
        lh_text = '迭代: %d | deepfaker.xyz | tonynostark' % (iter) if iter != 0 else ''
