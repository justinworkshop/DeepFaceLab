...
class AMPModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run():
            resolution = io.input_int("像素 Resolution", default_resolution, add_info="64-640", help_message="More resolution requires more VRAM and time to train. Value will be adjusted to multiple of 32 .")
            resolution = np.clip ( (resolution // 32) * 32, 64, 640)
            self.options['resolution'] = resolution
            self.options['face_type'] = io.input_str ("脸类型 Face type", default_face_type, ['f','wf','head'], help_message="whole face / head").lower()
        ...
        if self.is_first_run():
            ...
            self.options['inter_dims'] = np.clip ( io.input_int("内部编码器大小 Inter dimensions", default_inter_dims, add_info="32-2048", help_message="Should be equal or more than AutoEncoder dimensions. More dims are better, but require more VRAM. You can fine-tune model size to fit your GPU." ), 32, 2048 )
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['gan_power'] = np.clip ( io.input_number ("GAN强度 GAN power", default_gan_power, add_info="0.0 .. 5.0", help_message="Forces the neural network to learn small details of the face. Enable it only when the face is trained enough with random_warp(off), and don't disable. The higher the value, the higher the chances of artifacts. Typical fine value is 0.1"), 0.0, 5.0 )

            if self.options['gan_power'] != 0.0:
                gan_patch_size = np.clip ( io.input_int("GAN补丁大小 GAN patch size", default_gan_patch_size, add_info="3-640", help_message="The higher patch size, the higher the quality, the more VRAM is required. You can get sharper edges even at the lowest setting. Typical fine value is resolution / 8." ), 3, 640 )
            ...
            self.options['ct_mode'] = io.input_str (f"颜色转换模式 Color transfer for src faceset", default_ct_mode, ['none','rct','lct','mkl','idt','sot'], help_message="Change color distribution of src samples close to dst samples. If src faceset is deverse enough, then lct mode is fine in most cases.")
    ...
    def get_MergerConfig(self):
        morph_factor = np.clip ( io.input_number ("变形因子", 1.0, add_info="0.0 .. 1.0"), 0.0, 1.0 )
